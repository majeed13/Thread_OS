...
public class Kernel
{
	...
	case INTERRUPT_SOFTWARE: // System calls
	    ...
	    case WAIT:
		// get the current thread id --- getmyTCB..get tid
		// let the current thread sleep in waitQueue under the 
		// condition = this thread id
	    	// * * * * ADDED BY MUSTAFA * * * *  
	    	myTcb = scheduler.getMyTcb();
	    	int child = waitQueue.enqueueAndSleep(myTcb.getTid());
		return child; // return a child thread id who woke me up
			// * * * * END OF ADDITION * * * * 
	    case EXIT:
		...
		// tell the Scheduler to delete the current thread (since it is exiting)
	    	...
	    	int parent = myTcb.getPid();
	    	waitQueue.dequeueAndWakeup(parent, myTid);
